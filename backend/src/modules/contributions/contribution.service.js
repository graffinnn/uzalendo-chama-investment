...
const recordContribution = async (input, actorId, isSelfRecorded = false) => {
  ...
  if (isSelfRecorded) {
    await sequelize.query(
      ...
      { replacements: [actorId, 'CONTRIBUTION_SELF_RECORDED', 'contributions', contributionId, auditDetails] }
    );
  } else {
    await sequelize.query(
      'INSERT INTO audit_logs SET performed_by_admin = ?, action = ?, target_table = ?, target_id = ?, details = ?',
      { replacements: [actorId, 'CONTRIBUTION_RECORDED', 'contributions', contributionId, auditDetails] }
    );
  }
